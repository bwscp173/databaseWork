...
DROP FUNCTION IF EXISTS student_withdraw(integer,CHAR(4));
...
DROP FUNCTION IF EXISTS log_withdrawn_exam_extry();
...
CREATE OR REPLACE FUNCTION check_exam_same_day()
RETURNS TRIGGER AS $$
BEGIN

	BEGIN
		
		IF 0 < (SELECT count(*) FROM entry JOIN exam on exam.excode = entry.excode WHERE new.sno = entry.sno AND exam.exdate = 
		(SELECT exdate FROM exam WHERE exam.excode = new.excode)) THEN
			RAISE NOTICE 'Wont add entry as another entry this student "%" is taking has an exam on the same day. for course "%"',new.sno,new.excode;
			RETURN NULL;
		ELSE
			RETURN NEW;
			RAISE NOTICE 'no exam clashes, adding student "%"',new.sno;
		END IF;
	EXCEPTION
		WHEN unique_violation THEN
			RAISE NOTICE 'check_exam_same_day, is throwing an error possibly pk already in db';
	END;
...
CREATE OR REPLACE TRIGGER check_exam_same_day
BEFORE INSERT ON entry
FOR EACH ROW
EXECUTE FUNCTION check_exam_same_day();
SELECT * FROM student;
...
BEGIN
    IF TG_OP = 'DELETE' THEN
		BEGIN
			...
			RAISE NOTICE 'adding sno "%" to cancel table',old.sno;
		EXCEPTION
			WHEN unique_violation THEN
...
RETURNS VOID AS $$
BEGIN
	IF NOT EXISTS(SELECT 1 FROM entry WHERE entry.excode = exam_to_cancel) THEN
...
CREATE OR REPLACE FUNCTION student_withdraw(target_sno INTEGER)
-- deletes all entrys with the matching sno
RETURNS VOID AS $$
BEGIN
	-- checking if there is a student to deleted and will raise a correct notice
	IF EXISTS (SELECT 1 FROM entry WHERE sno = target_sno ) THEN
		RAISE NOTICE 'withdrawing entry in entry with sno "%"', target_sno;
		DELETE FROM entry WHERE sno = target_sno;	
		RAISE NOTICE 'withdrew sno "%"',target_sno;
	ELSE
		RAISE NOTICE 'cannot withdraw student as with sno "%", as it does not exist', target_sno;
	END IF;
END;
$$ LANGUAGE PLPGSQL;
